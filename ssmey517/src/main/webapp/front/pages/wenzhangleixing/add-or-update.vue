<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(41, 40, 40, 1)","textAlign":"left"}' class="title">文章类型</view>
				<input :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(40, 38, 38, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.wenzhangleixing" v-model="ruleForm.wenzhangleixing" placeholder="文章类型"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`wenzhangleixing`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																if((!this.ruleForm.wenzhangleixing)){
					this.$utils.msg(`文章类型不能为空`);
					return
				}
																																								if(this.ruleForm.id){
					await this.$api.update(`wenzhangleixing`, this.ruleForm);
				}else{
					await this.$api.add(`wenzhangleixing`, this.ruleForm);
				}
